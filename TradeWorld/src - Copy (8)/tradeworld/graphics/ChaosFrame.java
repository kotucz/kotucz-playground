...
public class ChaosFrame {
    ...
    private Action selectedAction = Action.BUILD_ROAD;
...
//                System.out.println("move");
                ...
                setEye(eye);
                {
                    ...
                    switch (selectedAction) {
                        case BUILD_ROAD:
                            cursor.setPos(new Point3d(
                                    Math.round(pick.x),
                                    Math.round(pick.y), 0));
                            cursor.setColor(new Color3f(0.5f, 0.5f, 0.5f));
                            cursor.setSize(0.5, 0.5);
                            break;
                        case BUILD_FACTORY:
                            Point3d pos = new Point3d(0.5 * selectedBuildingType.getWidthx() + Math.round(pick.x - selectedBuildingType.getWidthx() / 2.0), 0.5 * selectedBuildingType.getWidthy() + Math.round(pick.y - selectedBuildingType.getWidthy() / 2.0), 0);
                            cursor.setPos(pos);
                            cursor.setColor(
                                    (world.getLand().isAreaFree(Math.round(pick.x - selectedBuildingType.getWidthx() / 2.0), Math.round(pick.y - selectedBuildingType.getWidthy() / 2.0),
                                    selectedBuildingType.getWidthx(), selectedBuildingType.getWidthy())) ? new Color3f(0.0f, 1f, 0.0f) : new Color3f(1f, 0f, 0.0f));
