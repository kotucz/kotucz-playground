...
public class PlayActivity extends Activity {
	...
	GameBoard board;// = new GameBoard(this, 10, 14);
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		board = new GameBoard(this, 30, 18, null);
...
//			particleSystem.update(sx, sy, now);
			...
			canvas.drawOval(new RectF(w / 2f - one, h / 2f - one, w / 2f + one,
					h / 2f + one), p);

			canvas.drawLine(w / 2f, h / 2f, w / 2f + one * sx, h / 2f + one
					* sy, p);
			...
			if (motionEvent != null) {
				...
				Log.e("TOUCH", "hs: " + historySize + " pc: " + pointerCount);
				...
				for (int i = 0; i < motionEvent.getPointerCount(); i++) {
					...
					canvas.drawOval(new RectF(px - one, py - one, px + one, py
							+ one), p);
				...
				canvas.drawText(getActionText(), 100, 100, fontp);
