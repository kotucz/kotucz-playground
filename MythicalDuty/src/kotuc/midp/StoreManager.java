...
//            throw new Exception("StoreManager: recordIdentifier is null");
//        }
        storeData = null;
        position = 0;
        procIndex = 0;
    ...
    public long data(long i)
    {
        ...
        if(procOp == 0)
        {
            ...
            storeData[position++] = (byte)(i >> 16 & 0xff);
            storeData[position++] = (byte)(i >> 24 & 0xff);
            storeData[position++] = (byte)(i >> 32 & 0xff);
            storeData[position++] = (byte)(i >> 40 & 0xff);
            storeData[position++] = (byte)(i >> 48 & 0xff);
            storeData[position++] = (byte)(i >> 56 & 0xff);
        } else
        if(loaded)
            j = getFullByte() 
            + 256 * getFullByte() 
            + 0x10000 * getFullByte() 
            + 0x1000000 * getFullByte() 
            + 0x100000000L * getFullByte() 
            + 0x10000000000L * getFullByte() 
            + 0x1000000000000L * getFullByte() 
            + 0x100000000000000L * getFullByte();
        return j;
    }
    ...
    public static String recordIdentifier = null;
