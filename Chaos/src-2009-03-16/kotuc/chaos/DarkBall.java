...
class DarkBall extends PhysicEntity {

//	TransformGroup objHeadTG = null;
    private BranchGroup objBall = null;
...
//		objBall.addChild(fog);
        ...
        PointSound sound = new PointSound(new MediaContainer("file:./sounds/roar.au"), 1.0f, 0.0f, 0.0f, 0.0f);
...
//		partsAppear.setMaterial(new Material());
        ...
        addChild(new Shape3D(particles, partsAppear));


        setVelocity(new Vector3d(0.1 * (Math.random() - 0.5), 0.1 * (Math.random() - 0.5), 0));

//		createCollisionDetector();
    }
    private GeometryArray particles;
