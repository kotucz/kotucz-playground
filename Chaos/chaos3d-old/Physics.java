...
//		tSet = new Transform3D(this.angle, new Vector3d(this.pos), 1.0);
	...
	public List collisions () {
		...
		while (it.hasNext()) {
			Physics p;
			if (!((p=(Physics)it.next()).equals(this))
//				&&(p.hitpoints>0)
...
// gravity 		
		if (gravityCenter==null) {
			if (this.pos.z>1) this.vel.scaleAdd(period, gravity, this.vel);
			if (this.pos.z<1) {
				this.pos.z=1;
				this.vel.z=0;
			}
...
//	shows this physics velocity		
	...
	/**
	 *	@RETURNS this object surface normal at position p<br>normal is oriented out with length 1.0m
	...
	/**
	 *	@RETURNS this objects point nearest to position p
	 *	@PARAM p position in local world
	 */
