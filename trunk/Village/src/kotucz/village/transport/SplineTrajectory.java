...
//    private final double hand = 0.25; // hand right positive
    ...
    public SplineTrajectory(LinkedList<RoadPoint> path) {
        this(path, 0);
    }

    public SplineTrajectory(LinkedList<RoadPoint> path, float hand) {
        this.path = path;
...
//            points.add(toHandPoint(i, hand));
            points.add(path.get(i).getPosVector());
        }
...
//        return pos0;
    ...
    Vector3f toHandPoint(int i, float hand) {
        Vector3f pos1 = path.get(clamp(i - 1)).getPosVector();
        Vector3f pos2 = path.get(clamp(i + 1)).getPosVector();
        Vector3f vec = new Vector3f(pos2.y - pos1.y, pos1.x - pos2.x, 0);
        Vector3f pos = new Vector3f(path.get(clamp(i)).getPosVector());
