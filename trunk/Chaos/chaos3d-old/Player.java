...
class Player extends Soldier3D implements MouseMotionListener, KeyListener, MouseListener {
	...
	TransformGroup objRightHandTG = null;
	TransformGroup objLeftHandTG = null;
...
//		location.player.rightAttackBounds.getLocalToVworld(tSet);
		...
		Transform3D tSet = this.physics.getTransform(); 		
...
//		spotLight.setCapability(SpotLight.ALLOW_ENABLE_WRITE);
		auraLight.addScope(location.objLocation);
...
*/		
		if (e.getX()>lastx) turnRight(); 
		if (e.getX()<lastx) turnLeft();

		if (e.getY()<lasty) turnUp(); 
		if (e.getY()>lasty) turnDown();
