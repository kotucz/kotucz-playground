...
//        objFloor.setCollidable(false);
    ...
    public boolean isAreaFree(double x, double y, double wx, double wy) {

        for (int i = (int) Math.round(x); i <= (int) Math.round(x + wx); i++) {
            for (int j = (int) Math.round(y); j <= (int) Math.round(y + wy); j++) {
                if (roadNetwork.getPoint(i, j) != null) {
                    return false;
                }
            }
        }
