...
import tradeworld.actions.CreateObjectAction;
...
//        }
    ...
    public Soldier respawnSoldier(PlayerId playerid) {
        Soldier soldier1 = new Soldier(new Point3d(20 * Math.random(), 20 * Math.random(), 0));
...
//                    String name = event.getComponent().getIdentifier().getName();
                    ...
                    if ("x".equals(name)) {
                        ...
                        pan += -0.1 * Math.signum(value) * Math.pow(value, 2);
...
//            System.out.println("controller " + controller.getName());
            ...
            if (controller.getType() == Controller.Type.MOUSE) {
                ...
                for (Component component : controller.getComponents()) {
                    ...
                    if ("x".equals(name)) {
                        pan += -0.001 * Math.signum(value) * Math.pow(value, 2);
                    } else if ("y".equals(name)) {
                        tilt -= 0.001 * value;
                        tilt = Math.max(-1, Math.min(tilt, 1));
                    } else if (value > 0.5) { // pressed
                        if ("Left".equals(name)) {
                            fire |= true;
                        }
...
//            pos.z = 17;
                ...
                sight =
                        new Vector3d(Math.cos(pan), Math.sin(pan), tilt);
