...
public class ClientImpl implements Client {
    ...
    private Client stub; // stub is otherwise garbage collected

    public ClientId join(Server server) throws RemoteException {

        this.stub = (Client) UnicastRemoteObject.exportObject(this, 0);
...
//        registry.rebind(stub.toString(), stub); // not common
    ...
    public Time synchronize(Time serverTime) throws RemoteException {
        Time clientTime = game.getCurrentTime();
        this.game.setTime(serverTime);
        return clientTime;
    }
}
