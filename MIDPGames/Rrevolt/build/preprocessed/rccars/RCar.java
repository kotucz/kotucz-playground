...
//    float speed = 0;
    ...
    public RCar(RCTrack track) {
        this.track = track;
//        carG.addChild(RCanvas.loadObject("/cars/rc_auto1.m3g", CAR_M3G_ID));
...
//            speed = 0;
    ...
    public void moveCar() {
        ...
        rotZ += ((acc<0)?-1:1)*turn*getSpeed()/30;
...
// this is only free ride normaly = false!
        if (false) {

            vx += (acc * (float) Math.cos(Math.toRadians(rotZ + 90)));
            vy += (acc * (float) Math.sin(Math.toRadians(rotZ + 90)));
            ...
            if (onground) { // cannot accelerate with wheels in the air
                vx += (acc * (float) Math.cos(Math.toRadians(rotZ + 90)));
                vy += (acc * (float) Math.sin(Math.toRadians(rotZ + 90)));
